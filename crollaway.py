...
def gm(a):
    try:
        ...
        driver.get(url)
        box = driver.find_element_by_xpath(
            '/html/body/div[2]/div[2]/div[2]/div[3]/form/div[1]/div/input[3]')
        btn = driver.find_element_by_xpath(
            '/html/body/div[2]/div[2]/div[2]/div[3]/form/div[1]/div/a')
        box.send_keys(a)
        btn.click()
        html = driver.page_source
        soup = BeautifulSoup(html, 'html.parser')
...
def guri(a):
    try:
        driver.implicitly_wait(15)
        url = "https://www.guri.go.kr/www/selectGosiNttList.do?key=387&searchGosiSe=01,04,06"
        driver.get(url)
        box = driver.find_element_by_xpath(
            "/html/body/div[2]/div/div[2]/main/article/div/div/div[2]/form/fieldset/div/div/input")
        btn = driver.find_element_by_xpath(
            '/html/body/div[2]/div/div[2]/main/article/div/div/div[2]/form/fieldset/div/div/span[2]/button')
        box.send_keys(a)
        btn.click()
        html = driver.page_source
        soup = BeautifulSoup(html, 'html.parser')
        search_result = soup.select_one(
            '#contents > div > div.table-responsive > table > tbody')
        if not search_result:
            print("\n구리시청 " + a + "에 관한 최신공고 : ")
        else:
            onlytext = search_result.getText()
